// Package docker provides utilities for interacting with Docker and Docker Compose.
// It handles version detection, Docker Compose configuration generation, and execution
// of Docker commands for the Rayls CLI tool.
package docker
...
// CtxIsLogCmdKey is a context key for storing whether a command is a log command.
type CtxIsLogCmdKey struct{}

// CtxComposeVersionKey is a context key for storing the Docker Compose version.
type CtxComposeVersionKey struct{}

// DockerComposeVersion represents the version of Docker Compose detected on the system.
type DockerComposeVersion int

const (
	// None indicates Docker Compose is not available.
	None DockerComposeVersion = iota
	// ComposeV1 indicates docker-compose (v1) is installed.
	ComposeV1
	// ComposeV2 indicates docker compose (v2) is installed.
	ComposeV2
)

// String returns the string representation of the Docker Compose version.
func (d DockerComposeVersion) String() string {
	switch d {
	case ComposeV1:
		return "Compose v1"
	case ComposeV2:
